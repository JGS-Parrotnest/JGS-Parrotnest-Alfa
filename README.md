...
Parrotnest is a real‑time chat application built for classrooms, small communities, and friend groups.  
It combines:

- 🖥 **Windows desktop host** (WinForms shell) that runs the server (`ParrotnestServer.exe`)
- 🌐 **ASP.NET Core** API with SignalR for real‑time messaging
- 📄 **PHP + Vanilla JS** client served directly by the server
- 💾 **SQLite** database for users, friendships, messages, groups and admin logs

The goal is to provide a “Discord‑like” experience that is:

- easy to deploy on a single Windows machine,
- self‑contained (no external DB required by default),
...
- `Server/` – C# backend (`ParrotnestServer.csproj`, WinForms host + ASP.NET Core app)
- `Client/` – PHP/HTML/JS frontend (chat UI, login, settings, etc.)
- `Server/database.sql` / `Server/BAZA_DANYCH.md` – database schema and documentation
...
- 🖼 **Rich messaging**
  ...
  - Replying to messages (thread‑like UX)
...
- 📡 **Real‑time transport**
  ...
  - JWT‑based authentication for both REST and SignalR
...
- **Frontend**
  - PHP templates (`index.php`, `login.php`, …) served as static files
  - Vanilla JavaScript (`Client/app.js`) – all chat logic lives here
  - HTML/CSS UI, with CSS assets generated by a helper tool (`Client/CssGen`)

- **Database**
  - SQLite (`parrotnest.db`), auto‑created via `EnsureCreated()`
...
│  ├─ app.js                # Frontend logic (SignalR, uploads, UI)
│  ├─ uploads/              # Uploaded files & avatars (created at runtime)
│  └─ CssGen/               # CSS generator utility (C#)
├─ Server/
│  ├─ Controllers/          # ASP.NET Core controllers (Users, Messages, Files, Admin, ...)
│  ├─ Hubs/ChatHub.cs       # SignalR hub (real‑time messaging)
│  ├─ Data/ApplicationDbContext.cs # EF Core DbContext
│  ├─ Models/               # EF entities (User, Message, Group, ...)
│  ├─ ServerHost.cs         # Configures Kestrel, DI, routing, static files
│  ├─ ServerControlForm.cs  # WinForms host (Start/Stop, Open browser)
│  ├─ Program.cs            # Entry point (GUI or headless)
│  ├─ ParrotnestServer.csproj
...
> 📝 Note: Commands assume you have the .NET SDK installed and are running them from the repository root (`Alfa v8`).
...
- Click **“Start Server”** to launch the embedded ASP.NET Core server.
- Click **“Otwórz App”** to open `http://localhost:6069/login.php` in your browser.
...
1. In the chat input, the user clicks the 📎 attachment button.
2. `Client/app.js` reads the file from `<input type="file" id="imageInput">`.
3. If the file is small (≤ 5 MB), the client:
...
For large files (> 5 MB) the client uses a **three‑step chunked upload**:
...
- SQLite file at `parrotnest.db`, path resolved by `ServerHost.ResolveDbPath`:
  - Uses environment variables:
    - `PARROTNEST_DB_PATH` – override location
    - `PARROTNEST_DB_PREFER_APPDATA` – prefer `%LocalAppData%\ParrotnestServer\parrotnest.db`
...
- `Users` — accounts, avatars, preferences, admin flag
- `Friendships` — friend relationships with `Status` enum (`Pending`, `Accepted`, `Blocked`)
- `Groups` — chat groups with `OwnerId`
- `GroupMembers` — users in groups
- `Messages` — messages, references to sender/receiver/group, image URL, timestamp
- `AdminActionLogs` — admin audit log
- `GeneralChannelSettings` — settings for the global/general channel
...
This repository is designed to be hackable and school‑friendly:

- Customize the **PHP templates** in `Client/` to change branding and layout.
- Tweak **CSS** and themes (look for theme and text‑size settings).
- Extend **C# controllers** to add new endpoints (e.g. extra admin tools).
- Add new **SignalR hub methods** for richer real‑time interactions.

Ideas:

- 🔔 Add richer notification settings per‑user
- 📁 Add per‑group file repositories
- 🎨 Add more themes and per‑group color schemes
- 📊 Add basic analytics for admins (message counts, active users, etc.)
